...
echo "Deploying cloud function from ${SOURCE} to ${NMEA_CLOUDFUNC} "
...
echo "Deploying cloud function from ${SOURCE} to ${DECODE_CLOUDFUNC} "
gcloud functions deploy ${DECODE_CLOUDFUNC} \
  --entry-point=ais_stream_decode_pubsub \
  --runtime=python37 \
  --trigger-topic=NMEA_PUBSUB_TOPIC \
